...
const HeroSection = () => {
  ...
  return (
    <div
      className="overflow-x-hidden w-[100%] pt-10 min-h-[700px] dark:text-gray-200 bg-gray-200 dark:bg-gray-900 dark:transition-all dark:duration-500 tranistion-all duration-500 flex items-center "
      style={bgImage}
    >
      {/* ------------------  content container -------------------- */}
      ...
      <div className="w-11/12 mx-auto  flex md:flex-row flex-col gap-y-16 mmd:gap-y-0 md:justify-between pl-4 mt-10 pb-6">
        ...
        <div className="flex flex-col justify-center items-center md:items-start md:w-[50%] sm:gap-y-8 gap-y-6 ">
          <div className="flex flex-col items-center  md:gap-y-5 gap-y-3 dark:trainstion-all dark:duration-300 fade-anime py-6">
            <h1 className="sm:text-5xl xxs:text-4xl text-3xl font-semibold md:w-[450px] text-center md:text-left ">
              {title}{' '}
            </h1>
            <p className="text-right md:text- md:pr-8  text-primary font-semibold dark:text-gray-300">
              by {author || 'Anonymous'}
            </p>
            <p className="mt-4 text-sm font-[500] dark:text-gray-300">{description}</p>
          </div>

          {/* ------------ Order button ------------- */}

          <div>
            <button className="flex items-center gap-x-2 font-semibold px-3 py-2 rounded-3xl blue-gradient border border-white hover:border dark:border dark:border-black dark:hover:border-white dark:transition-all dark:duration-500   hover:border-blue-700 transition-all duration-400"
            //  onClick={()=> openModal()}
             >
              Order Now
            </button>
        ...
        <div className=" md:w-[40%] flex mmd:flex-row flex-col mmd:justify-between gap-y-5 mmd:gap-y-0 items-center">
          {/* ------------- book display part ------------- */}
